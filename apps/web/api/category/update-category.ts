...
export async function updateCategory(categoryId: string, form: UpdateForm): Promise<HttpResponse> {
...
export function validateUpdateCategory(form: UpdateForm): string | null {
   ...
   if (form.color && isValidHexcolor(form.color) === false) return `Given color - ${form.color} is not a valid color`;
